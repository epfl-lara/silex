...
/** Adds conversions from regular expressions to non-deterministic finite automata (NFAs)
  ...
  *
  * Expected to be mixed-in [[silex.RegExps]].
  *
...
trait Automatons { self: RegExps =>
  ...
  /** Transition of an NFA.
    ...
    * @group nfa
    */
  private[silex] sealed trait Transition {
  ...
  /** Transition guarded by a `predicate` on the next character.
    ...
    * @group nfa
    */
  private[silex] case class Guarded(condition: Condition, target: Int) extends Transition

  /** Epsilon transition. Can be freely taken.
    *
    * @group nfa
    */
  private[silex] case class Epsilon(target: Int) extends Transition

  /** Non-deterministic finite automaton.
    *
    * @group nfa
    */
  private[silex] trait NFA {
  ...
  /** Tree of values of type `A`, with branches predicated by a
    ...
    * @group dfa
    */
  private[silex] sealed trait DecisionTree[+A] {
  ...
  /** Predicate on a character.
    ...
    * @group dfa
    */
  private[silex] sealed trait Condition {
  ...
  /** Predicate based on a function.
    ...
    * @group dfa
    */
  private[silex] case class Predicated(predicate: Character => Boolean) extends Condition {
  ...
  /** Predicate based on equality.
    ...
    * @group dfa
    */
  private[silex] case class Valued(value: Character) extends Condition {
  ...
  /** Branching tree.
    ...
    * @group dfa
    */
  private[silex] case class Branch[+A](condition: Condition,
  ...
  /** Leaf tree. Always contains the same value for all characters.
    ...
    * @group dfa
    */
  private[silex] case class Leaf[+A](value: A) extends DecisionTree[A] {
