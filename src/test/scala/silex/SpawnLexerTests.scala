...
class SpawnLexerTests extends AnyFlatSpec with Lexers with CharLexers {
  ...
  it should "produce an error token in case of no match" in {
    ...
    assert(lexer.spawn(source("12AAX12")).toList == List("12", "error:A"))
  }
  ...
  it should "be able to continue after errors if specified" in {
    ...
    assert(lexer.spawn(source("12AAX12"), false).toList == List("12", "error:A", "error:A", "error:X", "12"))
  }
  ...
  it should "produce end tokens" in {
    ...
    assert(lexer.spawn(source("12AAX12"), true).toList == List("12", "error:A"))
    assert(lexer.spawn(source("12AAX12"), false).toList == List("12", "error:A", "error:A", "error:X", "12", "eof"))
  }
